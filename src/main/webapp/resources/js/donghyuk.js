	...
	var onCreate = function() {
		...
		$('.gnb li:nth-child(6)').click(function() {
			$.getJSON(app.context() + '/member/session', function(session){
				...
				$.ajax({
					url : app.context() + '/history/content',
					type : 'GET',
					data : {},
					dataType : 'JSON',
					success : function(data) {
						var BOOKING_FORM =
							...
							+'alt="지역"></th>'
							+'<td><select id="input_location" type="text"'
							+'class="input location" '
							+'style="width: 169px; height:27px;" value=""> '
							+'<option value="none">쏘카존을 선택하여 주세요.</option>';
							$.each(data.list, function(i,zone){
								BOOKING_FORM+='<option value="'+zone+'">'+zone+'</option>';
							});
							
							
						/*	+'<em id="input_location_zone_id" style="display: none;"></em> <em'
		...
		$('#btn_reserve_complete').click(function() {
			alert("성공성공");
			$.ajax({
				url : app.context() + '/history/reserve_result',
				type : 'GET',
				data : {},
				dataType : 'JSON',
				success : function(data) {
		...
		$('#btn_search_socar').click(function() {
			var id ='';
			var date = {
					'startDate' : $('#startDay').val(),
					'endDate' : $('#endDay').val(),
					'input_location' : $('#input_location > option:selected').val()
			};
			$.ajax({
				url : app.context() + '/history/select_date',
				type : 'POST',
				...
				success : function(data) {
					...
					+'</table>'
					+'</div>'
					//테이브르브르브르브르브르브르브르ㅡ브르브르브르브르브르ㅡㅂ르
					$.each(data.carList, function(i,car){
						CAR_LIST_FORM+='<div class="section"><div class="arti" style="width: 13%"><span class="zone_info_popup" value="'+car.socarZone+'">'+car.socarZone+'</span> <em style="display: none;"></em>'
						+'</div>'
						+'<div class="carInfo" style="widows: 47%">'
						+'<p class="thumb">'
						+'<a href="#" class="carDetail"><img src="resources/img/booking/'+car.carImg+'"></a>'
						+'</p>'
						+'<div class="desc">'
						+'<span><strong>'+car.carType+'</strong></br><span>차량 옵션 : '+car.carOption+'</span></span>'
						+'</div>'
						+'</div>'
						...
						+'<dt>SO회원 할인가</dt>'
						+'<dd>'
						+'<strong id="price-s2">'+car.rentAmt+'</strong>'
						+'</dd>'
						+'<dt style="display: none;">프로모션 할인가</dt>'
						+'<dd style="display: none;">'
						+'<strong id="price-p2"></strong>'
						+'</dd>'
						+'</dl>'
						+'</div>'
						+'<div class="oil" style="width: 10%">'+car.driveAmt+'</div>'
						+'<div class="rvBtn" style="width: 9%">'
						+'<a onclick="booking.reservation('+car.carSeq+')" id="btn_reserve" class="btn_reserve" title=""><img src="resources/img/booking/btn_list_reservation.gif" alt="예약"></a>'
					...
					CAR_LIST_FORM+='</fieldset>'
					+'</form>'
					
					$('#search_result_table').empty();
					$('#search_result_table').append(CAR_LIST_FORM);
				
					
				},
				
				error : function(m) {
					alert("쏘카 찾기 중 에러 발생" + m);
				}
			});
			
			$('#btn_reserve').click(function(){
				$.ajax({
				url : app.context() + '/history/confirm',
				type : 'POST',
				data : '',
				contentType : 'application/json',
				dataType : 'JSON',
	...
	return {
		...
		reservation : function(carSeq) {
			...
			if(date.start == '' || date.end == ''){
				...
				var start_arr = date.start.split('-');
				var end_arr = date.end.split('-');
				var rent_date = (start_arr[2] - end_arr[2]) * -1;
				$.getJSON(app.context() + '/car/findOne/car_seq/' + carSeq, function(data) {
					var CONFIRM_FORM =
					...
					+'<td><strong>'+data.carType+'</strong></td>'
					+'</tr>'
					+'<tr>'
					...
					+'<td>'
					+'<strong>'+data.rentAmt+'</strong> 원'
					+'</td>'
					+'</tr>'
					+'<tr>'
					+'<th><img src="resources/img/booking/text_discount.png" alt="할인"></th>'
					+'<td>'
					+'<input id="discount_coupon" type="radio" name="discount" value="coupon" checked> 쿠폰'
					+'<input id="discount_none" type="radio" name="discount" value="none"> 미적용'
					...
					frame = CONFIRM_FORM;
					$.getJSON(app.context() + '/member/session', function(session){
						$.getJSON(app.context() + '/mycoupon/notUsedList/' + session.id, function(mc){
							$.each(mc.list, function(i, coupon) {
								frame += '<option value='+coupon.couponMasterSeq+','+coupon.couponMemberSeq+'>'+coupon.couponName+'</option>';
							});
							var CONFIRM_FORM_COUPON_END = 
								'</select>'
								+'</div>'
								+'</td>'
								+'</tr>'
								...
								+'<td>'+data.driveAmt+'원 /1km당 <span>※ 반납 후 주행거리에 따라 부과</span></td>'
								+'</tr>'
								+'<tr>'
								+'<th><img src="resources/img/booking/payment_card.gif" alt="결제카드"></th>';
							frame += CONFIRM_FORM_COUPON_END;
							$.getJSON(app.context() + '/card/find/id/' + session.id, function(card){
								frame +=
									...
									+'<td>'
									+'<strong class="total_price">'+data.rentAmt+'</strong>'
								...
								var price = data.rentAmt;
								$('#selbox_coupon').change(function() {
									if($('#selbox_coupon').val() == 0){
										price = data.rentAmt;
										$('.total_price').text(price);
									} else {
										$.getJSON(app.context() + '/coupon/find/coupon_master_seq/' + $('#selbox_coupon option:selected').val() + '/1', function(c){
											if(c.dcOption == '%'){
												price = data.rentAmt;
												price = Math.floor(data.rentAmt - ((data.rentAmt / 100) * c.dc));
												$('.total_price').text(price);
											} else if(c.dcOption == '-'){
												price = data.rentAmt;
												price = data.rentAmt - c.dc;
												if(price < 0){
													price = 0;
												}
												$('.total_price').text(price);
								...
								$('#btn_reserve_complete').click(function() {
									...
									var car_num = data.carNum;
									var zone = data.socarZone;
									var reserve_info = {
											startDate : date.start,
											endDate : date.end,
											carSeq : data.carSeq,
											couponMemberSeq : split[1],
											id : session.id,
											price : price
									};
									var reserve_info2 = {
									...
									if($('#selbox_coupon').val() != 0){
									$.ajax({
										url : app.context() + '/history/reserve',
										type : 'POST',
										data : JSON.stringify(reserve_info),
										dataType : 'JSON',
										contentType : 'application/json',
										success : function(data) {
											$.getJSON(app.context() + '/mycoupon/useFlag/' + reserve_info.couponMemberSeq, function(data2) {
												RESULT_FORM =
													   '<div class="complete">'
													   +'<div id="container">'
													   +'<div id="content">'
													   ...
													   +'<a id="booking_result_go_main"><img src="resources/img/booking/btn_home.gif" alt="메인으로" onclick="controller.main()"></a>'
													   +'<a id="booking_result_go_history"><img src="resources/img/booking/btn_check.gif" alt="예약확인" onclick="member.history()"></a>'
													   +'</div>'
													   +'<div class="group">'
													   +'<table cellspacing="0">'
													   +'<tbody><tr>'
													   +'<th><img src="resources/img/booking/payment_txt1.gif" alt="차량"></th>'
													   +'<td><strong>'+car_name+'</strong> '+car_num+'</td>'
													   +'</tr>'
													   +'<tr>'
													   +'<th><img src="resources/img/booking/payment_txt2.gif" alt="일정"></th>'
													   +'<td>'+date.start+' ~ '+date.end+'</td>'
									...
									} else if($('#selbox_coupon').val() == 0){
										$.ajax({
											url : app.context() + '/history/notCouponInsert',
											type : 'POST',
											data : JSON.stringify(reserve_info),
											dataType : 'JSON',
											contentType : 'application/json',
								...
								$('#discount_coupon').click(function() {
									$('#selbox_coupon').css('display', 'block');
								});
								
								$('#discount_none').click(function() {
									$('#selbox_coupon').css('display', 'none');
									price = data.rentAmt;
									$('.total_price').text(price);
									$('#selbox_coupon').val(0).attr("selected", "selected");
								});
							});
						});
					});
...
/*var CONFIRM_FORM =
	...
	$.each(data.couponList, function(i,coupon){
	CONFIRM_FORM+='<option value="'+coupon+'" selected="selected">'+coupon+'</option>'
});
+'</select>'
+'</div>'
+'</td>'
